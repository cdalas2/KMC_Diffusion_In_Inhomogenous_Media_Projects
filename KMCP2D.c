...
#define TAU_TRAPPED 0.86 /* time between hops inside nanodomain (s) */
#define TAU_HOP (TAU_TRAPPED/3) /* time between hops outside nanodomain (s) */
#define SCALE 1
#define COORDF 1.0
#define HOP_AVG (2.79)
...
for (int iter=0; iter < 10000000; iter++){
  ...
  while(t[0] <= TIME_MAX){ /* we will run the simulation for 4 hours */
    ...
    th = -TAU_HOP*log(u7);
    W[lambda] = (double)N[lambda]/(tt + th)/4;
  ...
  D = (double)(x*x + y*y)/4/tlambda/TOTAL_BACTERIA_NUM/(SCALE*SCALE);
  //  printf("\n");
  //  printf("%f\n",D);
  DSUM = DSUM + D;
  DAVG = DSUM/(iter+1);
